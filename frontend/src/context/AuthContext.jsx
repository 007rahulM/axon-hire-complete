...
//////////////////////  working  with local storage  lets the down one use cookies ///////////////////////////////
/////////////////////////////////////////////////////////////
...
export const AuthProvider = ({ children }) => {
  ...
  // On app load: Verify user is still logged in
  useEffect(() => {
    axiosInstance.get("/auth/verify")  // ← Check with backend
      .then((res) => {
        setUser(res.data.user);        // ← Get user info
        setIsLoggedIn(true);
      })
      .catch(() => {
        setIsLoggedIn(false);          // ← Token cookie expired or invalid
      })
      .finally(() => setLoading(false));
  }, []);

  // Login: Backend already set the cookie
  // Frontend just updates user state
  ...
  const googleLogin = async (googleToken) => {
    try {
      ...
      if (res.status === 200 || res.status === 201) {
        // Backend set cookie automatically
        // Just update user state
        login(res.data.user);  // ← Only pass user, no token
        navigate("/");
      }
    } catch (err) {
  ...
  // Update user (for role upgrade, etc)
  ...
  // Logout
  const logout = () => {
    axiosInstance.post("/auth/logout")  // ← Tell backend to clear cookie
      .finally(() => {
        setUser(null);
        setIsLoggedIn(false);
        navigate("/login");
  ...
  return (
    <AuthContext.Provider value={{ isLoggedIn, user, login, logout, loading, googleLogin, updateUser }}>
      {!loading && children}
    </AuthContext.Provider>
  );
};

// 🔧 FIX: Add this comment to suppress the ESLint warning
// eslint-disable-next-line react-refresh/only-export-components
export const useAuth = () => useContext(AuthContext);