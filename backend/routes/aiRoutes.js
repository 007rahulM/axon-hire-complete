...
async function performAnalysis(resumeUrl, jobId, mode = "auto") {
  try {
    ...
    //loger 
    logger.debug(`AI analysis started for job ${jobId}`);
    const resumeData = await parseResumeFromUrl(resumeUrl);
    const job = await Job.findById(jobId);
    if (!job) throw new Error("JOB_NOT_FOUND");
...
`;
      ...
      if (aiResult.success) {
        ...
        const discovery = extractedFacts.learningDiscovery || [];
        if (discovery.length > 0) {
          console.log(`🧠 LEARNING LOOP: ${discovery.length} new skills discovered.`);
          try {
            const bulkOps = discovery.map(item => ({
              updateOne: {
                filter: { canonical: item.canonical.toLowerCase().trim() },
                update: {
                  $set: { isApproved: true, category: item.category || "technical-skills" },
                  $addToSet: { synonyms: { $each: (item.synonyms || []).map(s => s.toLowerCase().trim()) } },
                  $setOnInsert: { weight: 1.0, createdAt: new Date() }
                },
                upsert: true
              }
            }));
            const result = await Skill.bulkWrite(bulkOps);
            console.log(`✅ Saved ${result.upsertedCount} new skills, updated ${result.modifiedCount} existing.`);

            // Throttled cache refresh (max once per 30s)
            if (!global.lastCacheRefresh || Date.now() - global.lastCacheRefresh > 30000) {
              await refreshSkillCache();
              global.lastCacheRefresh = Date.now();
              console.log("🔄 Skill cache refreshed.");
            }
          } catch (saveError) {
            console.error("❌ Learning loop failed:", saveError.message);
          }
        } else {
          console.log("ℹ️ No new skills discovered.");
        }

        // Remove learningDiscovery from response (we saved it already)
        delete extractedFacts.learningDiscovery;
      } else if (mode === "beta") {
        console.error("BETA ERROR: AI Carousel exhausted.");
        return { success: false, error: "STRICT_BETA_FAILED", detail: "Beta Mode requires high‑fidelity AI which is currently unavailable." };
      }
    }

    // Fallback to local extraction if AI failed
    if (!extractedFacts) {
      console.log("Fallback: Using Local ZMath & Keyword Mapping.");
      extractedFacts = {
        skills: resumeData.skills,
    ...
    // Score using the extracted facts
    const finalAnalysis = calculateV3Score(
      {
        skills: extractedFacts.skills || [],
        totalMonths: Number(extractedFacts.totalMonths) || 0,
    ...
    // Augment with summary and metadata
    const analysis = {
      ...finalAnalysis,
      matchScore: finalAnalysis.score,
      score: finalAnalysis.score,
    ...
    //logger info
    logger.info(`AI extraction success via ${providerName}`);
    console.log(`\n--- [V3 SCORECARD: ${job.title}] ---`);
    console.table({
      "Result Method": providerName.includes("Local") ? "LOCAL" : "AI",
      "Provider": providerName,
    ...
    console.log(`MATCHED: ${analysis.matchedSkills.join(", ")}`);
    console.log("------------------------------------\n");
...
router.post("/generate-questions", verifyToken, async (req, res) => {
  try {
    ...
    res.status(200).json({ data, meta: { provider: result.provider, mode } });
  } catch (err) {
    console.error("Route Error:", err.message);
    res.status(500).json({ message: "Generation failed." });
  }
...
// ----------------------------------------------------------------------------
// Route: POST /api/ai/generate-questions-stream
// ----------------------------------------------------------------------------
router.post("/generate-questions-stream", verifyToken, async (req, res) => {
  try {
    const { jobTitle, mode } = req.body;
...
// ----------------------------------------------------------------------------
// Route: POST /api/ai/evaluate-myself (self‑assessment)
// ----------------------------------------------------------------------------
router.post("/evaluate-myself", verifyToken, async (req, res) => {
  try {
    const { resumeUrl, targetRole, jobDescription } = req.body;
    ...
    const resumeData = await parseResumeFromUrl(resumeUrl);
    if (!resumeData || !resumeData.fullText) throw new Error("Could not extract text from resume.");

    const systemPrompt = `
      You are an expert Technical Recruiter and Career Auditor.
      CONTEXT:
      - Target Role: "${targetRole || "Software Engineer"}"
      ${jobDescription ? `- Job Description snippet: "${jobDescription.substring(0, 500)}"` : ""}

      EVALUATION GUIDELINES:
      1. IMPACT (1-10): Search for quantitative results. Penalize generic tasks.
      2. RELEVANCE (1-10): Does the resume prioritize the right tech stack?
      3. STRUCTURE (1-10): ATS readability, information density.

      CRITICAL ANALYSIS RULES:
      - "matchedSkills": Only include hard skills actually present.
      - "missingRequiredSkills": Identify deal‑breaker technologies for this role.
      - "executiveSummary": Concise, 2‑3 sentence recruiter note.

      STRICT OUTPUT FORMAT (JSON only):
      {
        "ratings": { "impact": 1-10, "relevance": 1-10, "structure": 1-10 },
        "matchedSkills": ["Skill1", ...],
        "missingRequiredSkills": ["Missing1", ...],
        "executiveSummary": "..."
      }
    `;

    const userPrompt = `Analyze this resume for ${targetRole}:\n"${resumeData.fullText.substring(0, 12000)}"`;
    const result = await generateJSON(systemPrompt, userPrompt);
    if (!result.success || !result.data) throw new Error("AI failed to return data");
...
// ----------------------------------------------------------------------------
// Route: POST /api/ai/analyze-v3 (optional, kept for compatibility)
// ----------------------------------------------------------------------------
router.post("/analyze-v3", verifyToken, async (req, res) => {
  try {
    const { resumeUrl, jobId, applicationId } = req.body;
    ...
    // Use the same extraction prompt as in performAnalysis but with different schema
    const systemPrompt = `
ACT AS: A strict resume JSON parser for an ATS system.
OBJECTIVE: Parse resume text into structured JSON with ZERO assumptions.
SECTION DETECTION RULES: Identify sections like EXPERIENCE, PROJECTS, SKILLS, etc.
DATE CALCULATION RULES: Use ONLY experience sections, ignore education dates.
LINK EXTRACTION RULES: Only links from PROJECTS or ACHIEVEMENTS.
SKILL EXTRACTION RULES: experience (from job descriptions), skillsList (from skills sections), generalSkills (rest).
STRICT OUTPUT RULES: Return ONLY JSON. Use empty arrays/0 if missing.
SCHEMA:
{
  "skillsBySection": { "experience": [], "skillsList": [], "generalSkills": [] },
  "projectLinks": [],
...
`;
    ...
    const result = await generateJSON(systemPrompt, `RESUME TEXT:\n${resumeData.fullText}`);
    if (!result.data || !result.data.skillsBySection) throw new Error("AI failed to extract structured entities.");

    // Learning loop: add every skill found to the global skill map
    const allFoundSkills = [
      ...result.data.skillsBySection.experience,
      ...result.data.skillsBySection.skillsList,
      ...result.data.skillsBySection.generalSkills
    ...
    if (allFoundSkills.length > 0) {
      for (const rawName of allFoundSkills) {
        ...
        await Skill.findOneAndUpdate(
          { canonical },
          { $set: { canonical, isApproved: true }, $addToSet: { synonyms: rawName } },
          { upsert: true }
        );
      }
      await refreshSkillCache(); // No throttle – this route is used infrequently
    }
...
// Export the function for use in other routes (e.g., applicationRoutes)
router.performAnalysis = performAnalysis;
module.exports = router;