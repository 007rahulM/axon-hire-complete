  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        boxShadow: "var(--shadow-lg)", my: "auto",
      }}>

        {/* Header */}
        <div style={{ padding: "20px 24px 16px", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "flex-start", gap: "14px" }}>
          <div style={{ width: "40px", height: "40px", borderRadius: "10px", background: "var(--accent-bg)", border: "1px solid var(--accent-mid)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
            <svg width="18" height="18" fill="none" stroke="var(--accent)" strokeWidth="2" viewBox="0 0 24 24">
              <rect x="4" y="2" width="16" height="20" rx="2"/><line x1="12" y1="2" x2="12" y2="4"/><line x1="8" y1="2" x2="8" y2="4"/><line x1="16" y1="2" x2="16" y2="4"/>
            </svg>
          </div>
          <div style={{ flex: 1 }}>
            <h2 style={{ fontSize: "16px", fontWeight: 700, letterSpacing: "-0.015em", color: "var(--text-1)", marginBottom: "3px" }}>
              Setup your organisation
            </h2>
            <p style={{ fontSize: "12px", color: "var(--text-3)" }}>
              Establish your company identity on AxonHire to start posting jobs
            </p>
          </div>
          <button onClick={handleCancel} style={{ width: "28px", height: "28px", borderRadius: "5px", border: "1px solid var(--border)", background: "var(--bg-surface)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: "var(--text-3)", flexShrink: 0 }}>
            <CloseIcon />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} style={{ padding: "20px 24px 24px", display: "flex", flexDirection: "column", gap: "14px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
            <div style={{ display: "flex", flexDirection: "column", gap: "5px" }}>
              <label style={{ fontSize: "12px", fontWeight: 500, color: "var(--text-2)" }}>Company name *</label>
              <input required name="companyName" onChange={handleChange} placeholder="Acme Corp" style={inp} onFocus={onF} onBlur={onB} />
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: "5px" }}>
              <label style={{ fontSize: "12px", fontWeight: 500, color: "var(--text-2)" }}>Recruiting email *</label>
              <input required type="email" name="contactEmail" onChange={handleChange} placeholder="careers@acme.com" style={inp} onFocus={onF} onBlur={onB} />
            </div>
          </div>

          {/* Row 2: Industry + Size + Location */}
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "12px" }}>
            <div style={{ display: "flex", flexDirection: "column", gap: "5px" }}>
              <label style={{ fontSize: "12px", fontWeight: 500, color: "var(--text-2)" }}>Industry</label>
              <input name="industry" onChange={handleChange} placeholder="Fintech, AI…" style={inp} onFocus={onF} onBlur={onB} />
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: "5px" }}>
              <label style={{ fontSize: "12px", fontWeight: 500, color: "var(--text-2)" }}>Size</label>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "5px" }}>
              <label style={{ fontSize: "12px", fontWeight: 500, color: "var(--text-2)" }}>HQ Location</label>
              <input name="location" onChange={handleChange} placeholder="Bangalore, India" style={inp} onFocus={onF} onBlur={onB} />
            </div>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
            <div style={{ display: "flex", flexDirection: "column", gap: "5px" }}>
              <label style={{ fontSize: "12px", fontWeight: 500, color: "var(--text-2)" }}>Website</label>
              <input name="website" onChange={handleChange} placeholder="https://acme.com" style={inp} onFocus={onF} onBlur={onB} />
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: "5px" }}>
              <label style={{ fontSize: "12px", fontWeight: 500, color: "var(--text-2)" }}>
                Logo URL <span style={{ color: "var(--text-3)", fontWeight: 400 }}>(optional)</span>
              </label>
              <input name="logo" onChange={handleChange} placeholder="https://acme.com/logo.png" style={inp} onFocus={onF} onBlur={onB} />
            </div>
          </div>

          {/* Company bio */}
          <div style={{ display: "flex", flexDirection: "column", gap: "5px" }}>
            <label style={{ fontSize: "12px", fontWeight: 500, color: "var(--text-2)" }}>Company bio</label>
            <textarea name="description" rows={3} onChange={handleChange}
              placeholder="Briefly describe your mission and culture…"
          ...
          <div style={{ paddingTop: "4px" }}>
            <button type="submit" disabled={loading}
              style={{ width: "100%", padding: "11px", borderRadius: "6px", background: loading ? "var(--bg-subtle)" : "var(--accent)", color: loading ? "var(--text-3)" : "white", border: "none", fontFamily: "Inter, sans-serif", fontSize: "13px", fontWeight: 600, cursor: loading ? "not-allowed" : "pointer", transition: "all 0.15s", display: "flex", alignItems: "center", justifyContent: "center", gap: "8px" }}>
              {loading ? (
                <><span style={{ width: "13px", height: "13px", border: "2px solid var(--text-3)", borderTopColor: "transparent", borderRadius: "50%", animation: "ax-spin 0.7s linear infinite", display: "inline-block" }} />Setting up…</>
              ) : "Complete setup & post job"}
            </button>
            <p style={{ textAlign: "center", fontSize: "11px", color: "var(--text-3)", marginTop: "10px" }}>
              By continuing you agree to our Recruiter Terms of Service
            </p>
          </div>
        </form>
      </div>
